...
def build_test_case(
    ...
    """
    ref_for_input = reference_text
    # timeout対策：長すぎる参照は先頭だけ
    if len(ref_for_input) > 12000:
        ref_for_input = ref_for_input[:12000] + "\n...(省略)"
    ...
    return LLMTestCase(
        ...
        retrieval_context=[ref_for_input],
        additional_metadata={
...
def main(stage: str, ref_mode: str, md_path: Path, ref_yaml_dir: Path, target_yaml_dir: Path,
    ...
    # メトリクス（参照は必ず存在するので faithfulness は有効化）
    ...
    for doc in target_docs:
        ...
        ge_reason = None
        for m in (metrics_data or []):
            if getattr(m, "name", "") == "参照↔YAML突合（チェックリスト準拠）":
