...
※ CheckFlowはPhase 1で「UIそのもの」を必須にしない場合、ドキュメント+JSONテンプレ入力から開始しても良い。
...
- CheckFlowをPhase 1でどの程度作るか（UI実装 vs JSONテンプレ入力）
...
- CheckFlowのスタックは「提案」であり、企画/要件の正ではない（How隔離ポリシーと矛盾しない）
