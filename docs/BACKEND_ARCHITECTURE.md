# バックエンド構造定義書: Gate Runner（ScoreAggregationManager）

> 本書は、QA4AIDDの「自動検証」側（Gate Runner）を、コード構造・入出力・拡張点の観点で定義する。
> 参照：`docs/PRD.md` / `docs/TECH_STACK.md` / `artifacts/planning/PLN-PLN-SPLIT-001/PLN-PLN-EXEC-001.md`
...
> 重要（事実関係）：`runner/aidd-gate.py` は内部で WARN を exit_code=1 としてレポートへ記録するが、
> **プロセスの終了コードとしては FAIL（2）以外は 0 を返す**（ソース末尾のコメント方針どおり）。
> したがってCIブロック条件は「FAILのみ」であり、WARNは可視化・改善促進のための扱いである。

### 2.2 ゲート実装（個別スクリプト）

現状リポジトリには、個別ゲートスクリプトが存在する（例：G1/G3/G4）。

- `runner/gates/g1_ambiguity.py`：曖昧語チェック（汎用ゲートとして実装あり）
- `runner/gates/g2_checklist_completion.py`：チェックリスト完了検証（※現状の状態は要確認。CI Minimal の実体は `runner/aidd-gate.py`）
- `runner/gates/g3_schema.py`：スキーマ検証（分割YAMLディレクトリ/単体ファイル）
- `runner/gates/g4_deepeval.py`：DeepEval（MD/YAML参照切替）
- `runner/gates/g5_trace.py`：トレーサビリティ（※現状の状態は要確認）
...
> **As-Is 注意**：`tools/checklist/CheckFlow` が出力する JSON は、上記 `checklistresults.json` とは別フォーマット（プロトタイプ）である。
> Runner（G2）へ接続する場合は、契約形式での出力（または変換）を用意する。
...
- `runner/gates/g2_checklist_completion.py` / `runner/gates/g5_trace.py` の実装差分を、
  `runner/aidd-gate.py` とどう統合するか（どちらを正にするか）
...
- CI Minimal の実体が `runner/aidd-gate.py` であることを明記し、嘘がない
- `runner/gates/*` の実装状況（未整備/要確認を含む）を、事実ベースで明記している
