# PRD: QA4AIDD Gate Runner + CheckFlow（Coach）

> 本PRDは、企画成果物群 `artifacts/planning/PLN-PLN-SPLIT-001/` を「企画の正（Why/What）」として固定し、
> このリポジトリ（ScoreAggregationManager）が提供する価値・スコープ・受入条件・データ契約を明文化する。
>
> 企画の代表参照：
>
> - `artifacts/planning/PLN-PLN-SPLIT-001/PLN-PLN-EXEC-001.md`（エグゼクティブサマリー）
> - `artifacts/planning/PLN-PLN-SPLIT-001/PLN-PLN-SOLUTION-001.md`（2層ソリューション）
> - `artifacts/planning/PLN-PLN-SPLIT-001/PLN-PLN-CFUI-001.md`（CheckFlow設計方針）
> - `artifacts/planning/PLN-PLN-SPLIT-001/PLN-PLN-SCOREPOLICY-001.md`（スコア運用）
...
最終判断を人（CheckFlow/Coach）が持ち、機械的検証を自動（Gate Runner）が担い、証跡をシステム（JSON/Allure）に残す。
...
- **CheckFlow / Coach**：人がチェック項目を読み、判断（Done/Abort ほか運用上の状態）と理由/証跡を記録する仕組み（UIまたは代替手段）
...
### 8.1 CheckFlow（Coach / 人の最終判断 + 教育/視座共有）
...
※ Phase 1 では CheckFlow を必須にせず、JSONテンプレ入力やスプレッドシート→JSON変換などで代替してもよい（ただし契約は守る）。
...
> **As-Is 注意**：現在のCheckFlow実装（`tools/checklist/CheckFlow`）は、契約ファイル `checklistresults.json` ではなく
> `checkflow-export-<timestamp>.json` を出力している（プロトタイプ）。
> Runner（G2）接続のためには、契約形式でのエクスポート（または変換ツール）が必要。
  ...
  "items": [
    {
      ...
      "reason": "...",
      "evidencerefs": [
        "artifacts/planning/PLN-PLN-SPLIT-001/PLN-PLN-EXEC-001.md#0",
      ],
    },
  ],
}
...
> 企画側の詳細（ウェイト合算・Fatal条件・リスクレベルしきい値等）は
> `artifacts/planning/PLN-PLN-SPLIT-001/PLN-PLN-SCOREPOLICY-001.md` を正とする。
...
本ツール自体が AI（G4: DeepEval, PF: Promptfoo）を使うため、（将来）チェックリスト資産の観点を自己適用する。

> As-Is 注意：`packs/checklists/CHK-PLN-AIDD-001.yaml` は現状このリポジトリに存在しないため、
> 参照は「未確定」。暫定の人手観点は `tools/checklist/SAM_checklist_taste.md` を参照する。
...
- CheckFlowの配布形態：完全ローカル（静的）か、簡易ホスティングか
...
- CheckFlow設計（企画）：`artifacts/planning/PLN-PLN-SPLIT-001/PLN-PLN-CFUI-001.md`
...
## 17. 現状の実装ステータス（As-Is / 2026-03-07時点）

本PRDは要件定義だが、**「現状できること/できないこと」を明記**し、ドキュメントに嘘が混入しないようにする。

### 17.1 Runner（CI Minimal）の実体

- pack駆動の最小Runnerは `runner/aidd-gate.py`。
- packの例は `packs/pln_pack/pln.pack.yaml`。
- exit code は **FAIL のときのみ 2**。WARN はレポートへ出すが **プロセス終了コードは 0**（CIを止めない）。

### 17.2 Pack（PLN pack）の現状

- `packs/pln_pack/pln.pack.yaml` は存在するが、現状は以下の不整合があるため **そのままでは失敗し得る**。
  - `artifacts.pln_pack` が参照する成果物パス（`artifacts/planning/PLN-PLN-*.yaml`）が、現状の成果物配置（`artifacts/planning/yaml/`）とズレている
  - `paths.rules_ambiguity` が `packs/pln_pack/rules/` を参照しているが、実体は `packs/rules/` に存在する
