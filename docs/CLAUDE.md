...
- AIDD企画段階チェックリスト：`packs/checklists/CHK-PLN-AIDD-001.yaml`（※現状このリポジトリには存在しないため参照は未確定）
...
> As-Is 注意：現状のCheckFlowプロトタイプ（`tools/checklist/CheckFlow`）は、契約ファイル `checklistresults.json` ではなく
> `checkflow-export-<timestamp>.json` を出力する。Runner接続には契約形式での出力（または変換）が必要。
...
> **【手順未確定のため要確認】以下はスクリプトの実装状況/手順が未確定のため、実行前に動作確認して追記すること。**
>
> - `id/issue_id.py`（ID採番）: 実装は存在するが、デフォルト引数の参照先パスが現状リポジトリ構造と不整合のため、実行前に整備が必要。
> - `runner/gates/g1_ambiguity.py`: 実装あり（汎用ゲート）。
> - `runner/gates/g2_checklist_completion.py` / `runner/gates/g5_trace.py`: 現状の状態は要確認（未整備の可能性）。CI Minimal のG2は `runner/aidd-gate.py` が担う（`docs/BACKEND_ARCHITECTURE.md` 参照）。
